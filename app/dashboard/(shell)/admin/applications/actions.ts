...
async function assertCallerIsAdmin(): Promise<{ adminId: string } | { error: string }> {
  ...
  if (profile?.role !== 'admin') {
    return { error: 'Only admins can review applications.' };
  }
...
export async function approveApplication(
  ...
  await service.from('notifications').insert({
    ...
    content: `You've been approved as ${role}. Welcome to Anxiety Esports.`,
    link: '/dashboard',
...
export async function rejectApplication(
  ...
  const { error: userError } = await service.from('users').update({ status: 'rejected' }).eq('id', applicantUserId);
  ...
  await service.from('notifications').insert({
    ...
    content: note || 'Your application has been reviewed.',
    link: '/apply/status',
    actor_id: auth.adminId,
  });
